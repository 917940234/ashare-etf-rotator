...
from core.utils import ensure_dir, first_day_after_week_end
...
def run_bt_backtest(
    ...
    class ComputeTarget(bt.Algo):
        ...
        def __call__(self, target) -> bool:
            ...
            # 生成“全量权重字典”（未目标持有的标的权重为0），以便 bt 真正卖出旧持仓
            full_target = {s: 0.0 for s in symbols}
            full_target.update({k: float(v) for k, v in tw.items()})

            # no-trade band：目标与当前偏离<band则不交易（允许留现金）
            adj = {}
            for s in symbols:
                w = float(full_target.get(s, 0.0))
                cw = float(pre_weights.get(s, 0.0))
                adj[s] = cw if abs(w - cw) < no_trade_band else w
            tw = adj
    ...
    executed_perm = res.backtest_list[0].strategy.perm
    records = pd.DataFrame(executed_perm.get("rebalance_records", []))
